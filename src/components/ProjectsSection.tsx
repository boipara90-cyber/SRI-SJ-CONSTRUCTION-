    ...
    <section id="projects" className="py-20 bg-[#dbe2ea] text-slate-800 relative border-t border-slate-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {[
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-all flex items-center gap-1.5 cursor-pointer shadow-xs ${
                  filter === tab.id
                    ? 'bg-emerald-600 text-white ring-2 ring-emerald-500/40 shadow-sm'
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
              {MAJOR_CLIENTS.map((client, idx) => (
                <div key={idx} className="p-2.5 rounded-xl bg-slate-200 border border-slate-300 text-center hover:border-emerald-400 transition-colors">
          ...
          {filteredProjects.map((project) => (
            <div
              ...
              className={`rounded-2xl bg-slate-100 border transition-all duration-300 overflow-hidden flex flex-col justify-between group shadow-md ${
                project.status === 'Ongoing' ? 'border-emerald-500/80 ring-1 ring-emerald-500/20' : 'border-slate-300'
            ...
            >
              ...
              <div className="relative h-48 bg-slate-200 overflow-hidden">
                ...
                <div className="absolute top-3 left-3 flex items-center gap-2">
                  <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full border backdrop-blur-xs flex items-center gap-1.5 ${
                    project.status === 'Ongoing'
                      ? 'bg-emerald-950/90 text-emerald-300 border-emerald-500/60 shadow-xs'
                ...
                {project.client && (
                  <div className="absolute bottom-3 left-3 right-3">
                    <span className="inline-block text-[11px] font-black tracking-wide uppercase px-2.5 py-1 rounded-md bg-slate-950/90 text-emerald-400 border border-emerald-500/40 shadow-sm truncate max-w-full">
              ...
              <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                <div>
                  <h3 className="text-lg font-black text-slate-900 group-hover:text-emerald-700 transition-colors font-['Space_Grotesk'] leading-snug">
                    {project.title}
                  </h3>
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 text-xs font-black text-emerald-700 hover:text-emerald-600 transition-colors cursor-pointer"
                  >
        ...
        <div className="mt-12 text-center p-8 rounded-2xl bg-slate-900 border border-slate-800 text-white shadow-xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 text-xs font-bold uppercase tracking-wider mb-3">
          ...
          <div className="mt-5 flex flex-col sm:flex-row items-center justify-center gap-3">
            <button
              onClick={onOpenQuoteModal}
              className="inline-flex items-center gap-2 px-6 py-3 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-black text-sm shadow-md cursor-pointer transition-all"
            >
